...
ОСНОВНЫЕ ПОНЯТИЯ:
	...
	Пулл реквест / pull request - это отправка ваших изменений в репозитории на проверку
	программистам.
...
*/
	...
	В этом уроке научимся добавлять файлы в репозиторий. 
		...
		Команда добавления всех файлов на загрузку:
			"git add ."
			ВАЖНО! Под загрузкой файла подразумевается подготовка файла к отправке - отправка отправка начинается когда файл закомичен. 
...
*/
		...
		ВАЖНО! Папка, в которой нет файлов либо файлы внутри нее игнорируются - при загрузки на другой репозиторий не переносятся. 
	*/
		...
		echo '<hr><hr><br><h3>Back - #6.4 - Ветвления и слияние
				...
				ВАЖНО! Любой публичный репозиторий может быть скачен как его владельцем, так и сторонними разрабочиками. 
				Для скачивания в GiHab нужно нажать на "code" и скачать через Git, используя ссылку. Клонировать репозиторий.  
				
				КЛОНИРОВАНИЕ РЕПОЗИТОРИЙ В ДРУГУЮ ПАПКУ:
					Перед тем как слонировать репозиторий нежуно выбрать папку, в которой он будет находиться. 
					"cd www" - команда перемещения в папку. 
					где, 
					"cd" - объявление перемещения
					"www" - название папки в которую идет перемещение
					"../" - команды выхода из папки. Сначала нужно выбрать папку в которую будет закладываться репозиторий с удаленки. 
					"git clone https://github.com/GITHUB-SANICH/DZ_GIT.git" - клонирование репозитория из GitHab
					где, 
					"git clone" - команда клонирования
					"https://github.com/GITHUB-SANICH/DZ_GIT.git" - ссылка на репозиторий
						
				ОБНОВЛЕНИЕ СОДЕРЖИМОГО КЛОНИРОВАННОГО РЕПОЗИТОРИЯ
					"git pull" - команда обновления содержимого репозитория 
				ВАЖНО! Пулл нужно производить из той папки в которую был клонирован репозиторий, иначе выдается ошибка.  
				*/
